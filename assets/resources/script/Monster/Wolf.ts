...
@ccclass('Wolf')
export class Wolf extends Component {
    ...
    update(deltaTime: number) {
        ...
        if(!this.deadStat && !this.isAttacking){
            ...
            if(distToPlayer<600){
                if ((Math.abs(monPosX - playerPosX) > (this.node.getComponent(UITransform).contentSize.x)+35) ){
           
                     this.playAnimation("wolfMove");
