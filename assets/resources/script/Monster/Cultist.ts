...
@ccclass('Cultist')
export class Cultist extends Component {
    ...
    update(deltaTime: number) {
        ...
        if(!this.deadStat && !this.isAttacking){
            ...
            if(distToPlayer<600){
                ...
                if ((Math.abs(monPosX - playerPosX) > (this.node.getComponent(UITransform).contentSize.x)+45) ){
           
                     this.playAnimation("cultMove");
    ...
    attack(){
        ...
        this.scheduleOnce(()=>{         //Raycast
            for(let i =0 ;i< 3;i++){
                let p1 = new Vec2(this.node.worldPosition.x-(15*this.directionVal), this.node.worldPosition.y);
